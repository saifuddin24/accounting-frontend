...
import { Search, Printer, TrendingUp, TrendingDown } from 'lucide-vue-next'
...
<template>
    <div>
        ...
        <div v-if="reportData" class="card overflow-hidden animate-fade-in divide-y divide-gray-100 dark:divide-gray-700">
             <!-- Header -->
             <div class="p-6 bg-gray-50 dark:bg-gray-700/30">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white text-center uppercase tracking-wider">
                    Income Statement
                </h3>
                <p class="text-sm text-gray-500 text-center mt-1">
                    For the period {{ reportData.period.start }} to {{ reportData.period.end }}
                </p>
            </div>
            ...
            <div class="p-6">
                ...
                <div class="space-y-3">
                    <div v-for="rev in reportData.revenues" :key="rev.account_code" class="flex justify-between items-center text-sm">
                        <span class="text-gray-600 dark:text-gray-300">{{ rev.account_name }}</span>
                        <span class="font-medium text-gray-900 dark:text-white">{{ formatCurrency(rev.amount) }}</span>
                    </div>
                </div>
                <div class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 flex justify-between font-bold">
                    <span>Total Revenues</span>
                    <span class="text-blue-600 dark:text-blue-400 underline decoration-double">{{ formatCurrency(reportData.total_revenues) }}</span>
                </div>
            </div>
            ...
            <div class="p-6 bg-gray-50/50 dark:bg-gray-800/30">
                ...
                <div class="space-y-3">
                    <div v-for="exp in reportData.expenses" :key="exp.account_code" class="flex justify-between items-center text-sm">
                        <span class="text-gray-600 dark:text-gray-300">{{ exp.account_name }}</span>
                        <span class="font-medium text-gray-900 dark:text-white">{{ formatCurrency(exp.amount) }}</span>
                    </div>
                </div>
                <div class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 flex justify-between font-bold">
            ...
            <div class="p-6 bg-gray-100/50 dark:bg-gray-700/50">
                <div class="flex justify-between items-center">
                    <div class="flex items-center gap-3">
                        <div :class="[
                            'p-2 rounded-lg',
                            reportData.net_income >= 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                        ]">
                    ...
                    <span :class="[
                        'text-2xl font-bold',
                        reportData.net_income >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                    ]">
