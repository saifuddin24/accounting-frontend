<script setup>
import { Layout, Printer, Search } from 'lucide-vue-next'
...
const formatCurrency = (val) =>
  val === 0 ? '-' : Number(val).toLocaleString(undefined, { minimumFractionDigits: 2 })
  ...
  <div class="max-w-[100vw] overflow-hidden">
    ...
    <div v-if="reportData" class="card overflow-x-auto animate-fade-in">
      <table class="w-full text-xs text-left border-collapse min-w-[1000px] sm:min-w-0">
        ...
        >
          <tr>
            <th rowspan="2" class="px-4 py-3 border border-gray-200 dark:border-gray-700">
              Account
            </th>
            <th
              colspan="2"
              class="px-4 py-2 border border-gray-200 dark:border-gray-700 text-center bg-gray-200/50 dark:bg-gray-700/50"
            >
              Trial Balance
            </th>
            <th
              colspan="2"
              class="px-4 py-2 border border-gray-200 dark:border-gray-700 text-center bg-blue-50 dark:bg-blue-900/20"
            >
              Income Statement
            </th>
            <th
              colspan="2"
              class="px-4 py-2 border border-gray-200 dark:border-gray-700 text-center bg-green-50 dark:bg-green-900/20"
            >
              Balance Sheet
            </th>
          </tr>
          <tr class="text-[10px]">
            <th class="px-2 py-2 border border-gray-200 dark:border-gray-700 text-right w-24">
              Dr.
            </th>
            <th class="px-2 py-2 border border-gray-200 dark:border-gray-700 text-right w-24">
              Cr.
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
          ...
          >
            ...
            <td
              class="px-2 py-2 border border-gray-100 dark:border-gray-700 text-right text-gray-500"
            >
              {{ formatCurrency(line.tb_debit) }}
            </td>
            <td
              class="px-2 py-2 border border-gray-100 dark:border-gray-700 text-right text-gray-500"
            >
              {{ formatCurrency(line.tb_credit) }}
            </td>
            <td class="px-2 py-2 border border-gray-100 dark:border-gray-700 text-right">
              {{ formatCurrency(line.is_debit) }}
            </td>
            <td class="px-2 py-2 border border-gray-100 dark:border-gray-700 text-right">
              {{ formatCurrency(line.is_credit) }}
            </td>
            <td class="px-2 py-2 border border-gray-100 dark:border-gray-700 text-right">
              {{ formatCurrency(line.bs_debit) }}
            </td>
            <td class="px-2 py-2 border border-gray-100 dark:border-gray-700 text-right">
              {{ formatCurrency(line.bs_credit) }}
            </td>
          </tr>
        </tbody>
        <tfoot class="font-bold bg-gray-50 dark:bg-gray-800">
          ...
          <tr class="text-gray-900 dark:text-white">
            ...
            <td class="px-2 py-3 border border-gray-200 dark:border-gray-700 text-right">
              {{ formatCurrency(reportData.totals.tb_debit) }}
            </td>
            <td class="px-2 py-3 border border-gray-200 dark:border-gray-700 text-right">
              {{ formatCurrency(reportData.totals.tb_credit) }}
            </td>
            <td class="px-2 py-3 border border-gray-200 dark:border-gray-700 text-right">
              {{ formatCurrency(reportData.totals.is_debit) }}
            </td>
            <td class="px-2 py-3 border border-gray-200 dark:border-gray-700 text-right">
              {{ formatCurrency(reportData.totals.is_credit) }}
            </td>
            <td class="px-2 py-3 border border-gray-200 dark:border-gray-700 text-right">
              {{ formatCurrency(reportData.totals.bs_debit) }}
            </td>
            <td class="px-2 py-3 border border-gray-200 dark:border-gray-700 text-right">
              {{ formatCurrency(reportData.totals.bs_credit) }}
            </td>
          </tr>
          <!-- Net Income balancing line -->
          <tr class="text-blue-600 dark:text-blue-400">
            <td class="px-4 py-2 border border-gray-200 dark:border-gray-700 italic">
              Net Income / (Loss)
            </td>
          ...
          <tr class="text-gray-900 dark:text-white bg-gray-100 dark:bg-gray-700">
            ...
            <td
              class="px-2 py-3 border border-gray-200 dark:border-gray-700 text-right underline decoration-double"
            >
              {{
