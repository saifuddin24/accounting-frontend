...
import { Search, Printer, CheckCircle, AlertOctagon } from 'lucide-vue-next'
        ...
        <div v-if="reportData" class="card overflow-hidden animate-fade-in">
             <div class="p-6 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30">
                <div class="flex justify-between items-center">
                    <div>
                        <h2 class="text-xl font-bold text-gray-900 dark:text-white">Run Date: {{ reportData.as_of }}</h2>
                    </div>
                    <div>
                        <span v-if="reportData.is_balanced" class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    <tr v-for="line in reportData.lines" :key="line.account_code" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                         ...
                         <td class="px-6 py-3 font-medium text-gray-900 dark:text-white">{{ line.account_name }}</td>
                         <td class="px-6 py-3 text-right">{{ line.debit ? formatCurrency(line.debit) : '-' }}</td>
                         <td class="px-6 py-3 text-right">{{ line.credit ? formatCurrency(line.credit) : '-' }}</td>
                    </tr>
                </tbody>
